...
namespace PhanMemThiDua2026
{
    public partial class Form24_XacMinhAdmin : Form
    {
        ...
        {
            // Đợi nạp tên tài khoản tự  (nếu được phép)
        ...
        private async Task<(bool isSuccess, string errorMessage)> KiemTraAdminAsync(string tenDangNhap, string matKhauNhap)
        {
            try
            {
                ...
                {
                    string userDecrypted = Module_BaoMatAES.GiaiMa(reader.GetString(0));
                    string passDecrypted = Module_BaoMatAES.GiaiMa(reader.GetString(1));
        ...
        private async void btn_XacThuc_Click(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show(
                        $"Xác thực thất bại\nPhát hiện có nỗ lực truy cập trái phép!\nChính sách bảo mật đã được cấp quyền đóng phần mềm ngay lập tức",
                        "Thông báo",
        ...
        private void PictureBox1_Click(object? sender, EventArgs e)
        {
            MessageBox.Show("Vui lòng nhập mật khẩu xác thực để thực hiện thao tác xóa!",
                "Xác thực quyền admin", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        // 8. UX TỐI ƯU
