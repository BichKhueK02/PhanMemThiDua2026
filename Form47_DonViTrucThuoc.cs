...
{   
    public partial class Form47_DonViTrucThuoc : Form
    {
        ...
        public Form47_DonViTrucThuoc()
        {
            ...
            textBox_TenDonVi.Enter += TextBox_Enter;
            textBox_TenDonVi.Leave += TextBox_Leave;
            kryptonTextBox_KyHieuDonVi.Enter += TextBox_Enter;
            kryptonTextBox_KyHieuDonVi.Leave += TextBox_Leave;
        }
        ...
        private void InitToolTips()
        {
            ...
            {
                { kryptonButton1_Them, "Thêm đơn vị" },
                { kryptonButton1_Sua, "Sửa thông tin đơn vị" },
                { kryptonButton1_Xoa, "Xóa đơn vị" }
            };
        ...
        private async Task LoadDuLieuLenGridAsync()
        {
            ...
            await Task.Run(() =>
            {
                ...
                conn.Open();
                // Bạn có thể giữ nguyên câu SQL hoặc bỏ ORDER BY đi cho nhẹ vì đằng nào mình cũng sort lại trên C#
                using var cmd = new SqliteCommand("SELECT ID, STT, TenDonVi, KyHieu, ThoiGian FROM DanhSachDonVi_CapTrucThuoc", conn);
                ...
                // ⭐ SẮP XẾP A-Z TẠI ĐÂY (Sau khi đã giải mã xong)
                // Dùng StringComparer.CurrentCultureIgnoreCase để sắp xếp đúng bảng chữ cái, không phân biệt hoa thường
                dsMoi = dsMoi.OrderBy(x => x.TenDonVi, StringComparer.CurrentCultureIgnoreCase).ToList();
            });
        ...
        private async void kryptonButton1_Them_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    cmd.Parameters.AddWithValue("@kyhieu", Module_BaoMatAES.MaHoa(kyHieu));
        ...
        private async void kryptonButton1_Sua_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    cmd.Parameters.AddWithValue("@kyhieu", Module_BaoMatAES.MaHoa(kyHieuMoi));
                    cmd.Parameters.AddWithValue("@thoigian", DateTime.Now.ToString("dd/MM/yyyy HH:mm"));
        ...
        private void KryptonDataGridView1_SelectionChanged(object sender, EventArgs e)
        {
                ...
                kryptonDataGridView1.CurrentRow.Index >= 0)
            {
                ...
                if (idx < _danhSachDonVi.Count)
                {
                    ...
                    kryptonTextBox_KyHieuDonVi.Text = item.KyHieu;
                    DoiTenNutSua(laDangChonSua: true);
                    // Đưa con trỏ vào ô Tên đơn vị và đặt ở cuối nội dung
                    textBox_TenDonVi.Focus();
                    textBox_TenDonVi.SelectionStart = textBox_TenDonVi.TextLength;
                    textBox_TenDonVi.SelectionLength = 0;
                    return;
                }
            }
            XoaTrangOInput();
        ...
        private string GiaiMaSafe(string val)
        {
            ...
            try
            {
                string dec = Module_BaoMatAES.GiaiMa(val);
                return string.IsNullOrEmpty(dec) ? val : dec;
            }
            catch { return val; }
        }
        private void DatVienXanh(KryptonTextBox textBox)
        {
            textBox.StateCommon.Border.Color1 = Color.DodgerBlue;
        }
        private void XoaVienXanh(KryptonTextBox textBox)
        {
