            ...
            try
            {
                ...
                WHERE t.ID = 1 LIMIT 1";
                    ...
                    if (await reader.ReadAsync())
                    {
                        ...
                        string diaDiem = Dep(Module_BaoMatAES.GiaiMa(reader["DiaDiem"]?.ToString() ?? ""));
                        string ngay = Dep(Module_BaoMatAES.GiaiMa(reader["Ngay"]?.ToString() ?? ""));
                        string thang = Dep(Module_BaoMatAES.GiaiMa(reader["Thang"]?.ToString() ?? ""));
                        string nam = Dep(Module_BaoMatAES.GiaiMa(reader["Nam"]?.ToString() ?? ""));
                        string chiHuyD = Dep(Module_BaoMatAES.GiaiMa(reader["ChiHuyD"]?.ToString() ?? ""));
                        string deNghi = Dep(Module_BaoMatAES.GiaiMa(reader["LoaiDeNghi"]?.ToString() ?? ""));
