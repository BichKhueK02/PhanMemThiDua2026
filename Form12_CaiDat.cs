...
namespace PhanMemThiDua2026
{
    public partial class Form12 : Form
    {
        ...
        private void InitFocusHighlightForm12()
        {
            KryptonTextBox[] textBoxes =
            {
        ...
        kryptonTextBox1_DonViBanHanhToTrinh
            ...
            {
                textBox.GotFocus -= Form12_ControlGotFocus;
                textBox.LostFocus -= Form12_ControlLostFocus;
                textBox.GotFocus += Form12_ControlGotFocus;
                textBox.LostFocus += Form12_ControlLostFocus;
                if (!_originalBorderColors.ContainsKey(textBox))
                    _originalBorderColors[textBox] = textBox.StateCommon.Border.Color1;
            }
        }
        private void Form12_ControlGotFocus(object? sender, EventArgs e)
        {
            if (sender is not KryptonTextBox textBox)
                return;
            textBox.StateCommon.Border.DrawBorders = Krypton.Toolkit.PaletteDrawBorders.All;
            textBox.StateCommon.Border.Color1 = _focusColor;
            textBox.StateCommon.Border.Color2 = _focusColor;
            textBox.StateCommon.Border.Width = 2;
            textBox.Refresh();
        }
        private void Form12_ControlLostFocus(object? sender, EventArgs e)
        {
            if (sender is not KryptonTextBox textBox)
                return;
            if (_originalBorderColors.TryGetValue(textBox, out Color originalColor))
            {
                textBox.StateCommon.Border.Color1 = originalColor;
                textBox.StateCommon.Border.Color2 = originalColor;
            }
            ...
            ";
                    ...
                    if (reader.Read())
                    {
                        ...
                        if (!reader.IsDBNull(3))
                            comboBox1_ChonSuKienThoat.Text = Module_BaoMatAES.GiaiMa(reader[3].ToString());
                        else
...
";
        ...
        private string SaveToSQLiteCore(CauHinhData uiData)
        {
            ...
            try
            {
                // BỔ SUNG: BẢO ĐẢM BẢNG ĐƠN VỊ BAN HÀNH TỜ TRÌNH ĐÃ TỒN TẠI
                using (var cmdInit = cn.CreateCommand())
                {
                    cmdInit.Transaction = tran;
                    cmdInit.CommandText = @"
                CREATE TABLE IF NOT EXISTS ""DanhSach_DonViBanHanhToTrinh"" (
                    ""ID"" INTEGER NOT NULL,
                    ""TenDonVi"" TEXT,
        ...
        private async void kryptonButton_LuuThongTin_Click(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // 🚀 GỌI HÀM LƯU TỜ TRÌNH TẠI ĐÂY
                //Luu_DonViBanHanhToTrinh(kryptonTextBox1_DonViBanHanhToTrinh.Text);
        ...
        private static string SafeEncrypt(string value)
        {
            if (string.IsNullOrWhiteSpace(value))
                return "";
            var result = Module_BaoMatAES.MaHoa(value);
        ...
        private static string SafeDecrypt(object input)
        {
            ...
            try
            {
                return Module_BaoMatAES.GiaiMaNhanDang(value);
            }
            catch (Exception ex)
            {
        ...
        public static void LuuSuKienThoat(string luaChon)
        {
            ...
            string luaChonMaHoa = Module_BaoMatAES.MaHoa(luaChon);
            // Update ID = 1 nếu có
            using var cmdUpdate = cn.CreateCommand();
        ...
        CREATE TABLE IF NOT EXISTS SuKien_ThoatPhanMem (
            ...
            using (var cmdCheck = cn.CreateCommand())
            {
                ...
                if (count == 0)
                {
                    ...
                    using var cmdInsert = cn.CreateCommand();
                    string luaChonMaHoa = Module_BaoMatAES.MaHoa("Thoát ngay");
                    cmdInsert.CommandText = "INSERT INTO SuKien_ThoatPhanMem (SuKien_DuọcChon) VALUES (@luaChon)";
                    cmdInsert.Parameters.AddWithValue("@luaChon", luaChonMaHoa);
                    cmdInsert.ExecuteNonQuery();
                }
            ...
            return Module_BaoMatAES.GiaiMa(result.ToString());
        }
        ...
        private void HienThiFormAo_ThongTinPhanMem_Luoi(string tieuDe, string rawData)
        {
            ...
            using (var formAo = new FormAoBase())
            {
                ...
                formAo.Size = new System.Drawing.Size(1100, 580); // Cân đối với lượng thông tin Token
                formAo.FormBorderStyle = FormBorderStyle.FixedDialog;
                ...
                grid.Columns[0].DefaultCellStyle.Font = new System.Drawing.Font(
                    Module_HeThong.TenFontHeThong, 10F, System.Drawing.FontStyle.Bold);
                grid.Columns[0].DefaultCellStyle.ForeColor = System.Drawing.Color.FromArgb(60, 60, 60);
                grid.Columns[1].DefaultCellStyle.WrapMode = DataGridViewTriState.True;
                // --- 5. ĐỔ DỮ LIỆU & TÔ MÀU ---
    ...
    EventArgs e)
        ...
        private void KhoiTaoBang_DonViBanHanhToTrinh()
        {
            ...
            {
                conn.Open();
                // 1. Kiểm tra bảng tồn tại, nếu có rồi thì thoát ngay
                string checkTableSql = "SELECT count(*) FROM sqlite_master WHERE type='table' AND name='DanhSach_DonViBanHanhToTrinh';";
                using (var checkCmd = new SqliteCommand(checkTableSql, conn))
                {
                    long count = (long)checkCmd.ExecuteScalar();
                    if (count > 0)
                    {
                        return; // Bảng đã tồn tại -> Thoát ngay
                    }
                }
                // 2. Tạo bảng nếu chưa có
                string createTableSql = @"
            CREATE TABLE ""DanhSach_DonViBanHanhToTrinh"" (
                ""ID"" INTEGER NOT NULL,
                ""TenDonVi"" TEXT,
                PRIMARY KEY(""ID"" AUTOINCREMENT)
            );";
        ...
        private void Load_DonViBanHanhToTrinh()
        {
           KhoiTaoBang_DonViBanHanhToTrinh(); // Đảm bảo bảng đã tồn tại trước khi thao tác
            ...
            {
                conn.Open();
                string sql = "SELECT TenDonVi FROM DanhSach_DonViBanHanhToTrinh WHERE ID = 1;";
                using (var cmd = new SqliteCommand(sql, conn))
                {
                    ...
                    if (result != null && result != DBNull.Value)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(rawValue))
                        {
                            try
                            {
                                // Giải mã dữ liệu lấy từ SQLite
                                kryptonTextBox1_DonViBanHanhToTrinh.Text = Module_BaoMatAES.GiaiMa(rawValue);
                            }
                            catch
                            {
                                // Nếu dữ liệu cũ trong CSDL là text thuần chưa mã hóa
                                kryptonTextBox1_DonViBanHanhToTrinh.Text = rawValue;
                            }
                        }
                        else
                        {
                            kryptonTextBox1_DonViBanHanhToTrinh.Text = string.Empty;
                        }
                    }
                    else
                    {
